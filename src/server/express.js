...
app.get('cargarCategorias', (cond, res) =>{
    console.log('Se cargan todas las categorias WHERE='+cond);
		
    con.query("SELECT c.idCategoria, c.Nombre FROM `coetus-emere`.tcategorias c join `coetus-emere`.tproductos p on c.idCategoria = p.idCategoria join `coetus-emere`.tusuarios u on u.idUsuario = p.idProveedor where "+cond+" group by c.idCategoria", function (err, result, fields) {
        if (err) throw err;
